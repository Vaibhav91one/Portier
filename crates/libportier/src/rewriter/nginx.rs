...
impl NginxBackend {
    ...
    fn replace_port(line: &str, new_port: u16) -> String {
        ...
        if let Some(last_colon) = inner.rfind(':') {
            ...
            let rest = after_colon.split_whitespace().skip(1).collect::<Vec<_>>().join(" ");
            if rest.is_empty() {
                format!("{}listen {}{};", indent, before_colon_incl, new_port)
            } else {
                format!("{}listen {}{} {};", indent, before_colon_incl, new_port, rest)
            }
        } else {
...
mod tests {
    ...
    fn test_extract_port_bare_with_modifier() {
        assert_eq!(NginxBackend::extract_port("listen 80 default_server;"), Some(80));
    }
    ...
    fn test_extract_port_ipv6_with_modifier() {
        assert_eq!(NginxBackend::extract_port("listen [::1]:80 default_server;"), Some(80));
    }
    ...
    fn test_replace_port_bare() {
        assert_eq!(NginxBackend::replace_port("listen 80;", 3000), "listen 3000;");
    }

    #[test]
    fn test_replace_port_bare_with_modifier() {
        assert_eq!(NginxBackend::replace_port("listen 80 default_server;", 3000), "listen 3000 default_server;");
    }

    #[test]
    fn test_replace_port_ipv6() {
        assert_eq!(NginxBackend::replace_port("listen [::1]:80;", 3000), "listen [::1]:3000;");
    }

    #[test]
    fn test_replace_port_ipv6_with_modifier() {
        assert_eq!(NginxBackend::replace_port("listen [::1]:80 default_server;", 3000), "listen [::1]:3000 default_server;");
    }
